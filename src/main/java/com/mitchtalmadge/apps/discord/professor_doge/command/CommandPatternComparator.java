...
public class CommandPatternComparator implements Comparator<CommandPattern> {
    ...
    public CommandPatternComparator(Command command) {
     ...
     * @return -1 if strict1 matches but not strict2, 0 if both or neither match equally, and 1 if strict2 matches but not strict1.
     */
    private int compareBothStrict(CommandPattern strict1, CommandPattern strict2) {
        if (Arrays.equals(strict1.value(), command.getArgs())) {
            if (Arrays.equals(strict2.value(), command.getArgs()))
                return 0;
            return -1;
        } else {
            if (Arrays.equals(strict2.value(), command.getArgs()))
     ...
     * @return -1 if strict matches more than loose, 0 if both or neither match equally, and 1 if loose matches more than strict.
     */
    private int compareOneStrict(CommandPattern strict, CommandPattern loose) {
        if (Arrays.equals(strict.value(), command.getArgs())) {
            if (strict.value().length > loose.value().length)
                return -1;

            int currentArg = 0;
            while (currentArg < loose.value().length && loose.value()[currentArg].equals(command.getArgs()[currentArg])) {
                currentArg++;
            }
     ...
     * @return -1 if loose1 matches but not loose2, 0 if both or neither match equally, and 1 if loose2 matches but not loose1.
     */
    private int compareBothLoose(CommandPattern loose1, CommandPattern loose2) {
        ...
        for (int i = 0; i < command.getArgs().length; i++) {
            if (winsPoint(loose1, i))
                pattern1Score++;
            if (winsPoint(loose2, i))
                pattern2Score++;
        }
    ...
    private boolean winsPoint(CommandPattern pattern, int argIndex) {
        ...
        if (pattern.value().length > argIndex) {
            ...
            if (pattern.value()[argIndex].equalsIgnoreCase(command.getArgs()[argIndex])) {
                return true;
            }
        }
